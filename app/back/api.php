<?php
    ...
    class Api extends Rest{
        ...
        public function addUsuario(){
            ...
            // $cidade=$this->validateParameter('cidade',$this->param['cidade'],STRING,false);
            // $rotas=$this->validateParameter('rotas',$this->param['rotas'],STRING,false);
            // $imagem=$this->validateParameter('imagem',$this->param['imagem'],STRING,false);
        ...
        public function getUsuarioDetails() {
			...
			$response['rotas']=$usuario['rotas'];
			$response['imagem']=$usuario['imagem'];
			$this->returnResponse(SUCCESS_RESPONSE, $response);
		}
        
        public function getAllUsuariosDetails(){
            $aux = new Usuario;
            $aux->setId($usuario_status);
			$usuario = $aux->getAllUsuarios();
			if(!is_array($usuario)) {
				$this->returnResponse(SUCCESS_RESPONSE, ['message' => 'No User was found.']);
			}
        ...
        public function getRotaDetails() {
			...
			$response['imagem']=$rota['imagem'];
			$this->returnResponse(SUCCESS_RESPONSE, $response);
		}

        public function getAllRotasDetails(){
            $aux = new Rota;
            $aux->setId($rota_status);
			$rota = $aux->getAllRotas();
			if(!is_array($rota)) {
				$this->returnResponse(SUCCESS_RESPONSE, ['message' => 'No Routes were found.']);
			}
        ...
        public function getAllMensagensDetails(){
            $aux = new Mensagem;
            $aux->setId($mensagem_status);
			$mensagem = $aux->getAllMensagens();
			if(!is_array($mensagem)) {
				$this->returnResponse(SUCCESS_RESPONSE, ['message' => 'No Messages were found.']);
			}
